// satt-ihop-livets-molekyler.js – sätter ihop byggfiler för Organisk kemi 6 "Livets molekyler" ur leveransfilerna
// (doc/leveranser/livets-molekyler/, framtagna ur Joachims arbetsfiler av verktyg/rensa-livets-molekyler.js):
//   avsnitt-N.md                    (Standard: ingress före ---, "## N.X Titel", "### rubrik"; inga bildrutor)
//   avsnitt-N-enkel-fordjupning.md  ("# 📗 ENKEL" [med "## Avsnittsingress" = Enkel-ingressen] / "# 📕 FÖRDJUPNING", "## N.X Titel")
//   original/dk6-bildspecar.md      (bildspecar: "## M1 — Titel" + "*1.1, vid tredje rubriken*", alla SVG)
//   knapptitlar-och-ingresser.md    (valfri, Joachims beslut), karnpunkter.md (valfri), flipcards.md/kortsvar.md (valfria)
// → doc/leveranser/livets-molekyler/bygg/… i bygg-avsnitt.js-format. Bildankare per nivå ligger i bygg-avsnitt-konfig.js.
// Kör: node verktyg/satt-ihop-livets-molekyler.js [N…], sedan node verktyg/bygg-avsnitt.js livets-molekyler N.
// Kopia av satt-ihop-syror-estrar.js (arbetsorder 2026-09-19) med fyra avsnitt, k6-prefix och kortsvarsundantag (fråga 42); övriga regler lika.
'use strict';
const fs = require('fs'), path = require('path');
const { arReaktion } = require('./lib-notation.js');
const R = path.join(__dirname, '..', 'doc', 'leveranser', 'livets-molekyler');
...
// slug och hero-underrubrik per avsnitt (ordern 2026-09-19: knapptitlar och hero-underrubriker levereras separat; förslag av
// Code, rapporterade – ersätts av knapptitlar-och-ingresser.md när Joachim beslutar)
const HUVUD = {
  1: { slug: 'kolhydrater', sub: 'sockerringar, ensamma och i kedjor' },
  2: { slug: 'fetter', sub: 'glycerol, tre fettsyror och en knyck som avgör' },
  3: { slug: 'proteiner', sub: 'kedjor av aminosyror som viks till en form' },
  4: { slug: 'kvavet-vitaminerna-och-mineralerna', sub: 'det som inte blir koldioxid och vatten' }
};
const BOK = ['A', 'B', 'C'];
...
if (finns('karnpunkter.md')) {
  ...
  const n = Object.values(KP).reduce((x, l) => x + l.split('\n').length, 0);   // ordern 2026-09-19: 12 underdelar × 5 = 60
...
const rensa = t => t.split('\n').filter(l => !/^> /.test(l) && !/^\*ca \d+ ord\*$/.test(l.trim()) && !/^\*[^*]+undantaget[^*]*\*$/.test(l.trim()) && !/^\*{1,2}Djupdykning härifrån:/.test(l) && !/^Bildspecarna ligger inbakade i bildrutorna/.test(l) && !/^\*Fyra rubriker[^*]*\*$/.test(l.trim())).join('\n')   // även kursiv djupdykningsrad (avsnitt 3, djupdykningen inte skriven) och leveransnoten om bildspecar
  .replace(/\n{3,}/g, '\n\n').trim();
const fetaReaktioner = t => t.split(/\n\n+/).map(p => (!/\n/.test(p) && !/\*\*/.test(p) && arReaktion(p.trim(), { energi: true })) ? `**${p.trim()}**` : p).join('\n\n');   // "+ energi" som sista term (jäsningen, 2.2)
...
for (const N of AVSNITT) {
  ...
  for (const u of under) {
    ...
    for (const d of delar.slice(1)) {
      ...
      const fil = 'k6-' + h[1].toLowerCase() + (h[3].startsWith('AI') ? '.webp' : '.svg');
  ...
  // bildspecar ur original/dk6-bildspecar.md: "## M1 — Titel" + "*1.1, vid tredje rubriken*" (alla SVG)
  const BSdel = fs.readFileSync(path.join(R, 'original', 'dk6-bildspecar.md'), 'utf8').replace(/\r\n/g, '\n');
  for (const b0 of BSdel.matchAll(/\n## ([A-Z]\d+) — ([^\n]+)\n\*(\d)\.(\d), vid ([^*]+?)\*\n([\s\S]*?)(?=\n---\n|\n# )/g)) {
    const b = [b0[0], b0[1], b0[2], 'SVG', b0[3], b0[4], b0[5], b0[6]];   // samma fältordning som i satt-ihop-fossila.js
    if (+b[4] !== N) { continue; }
    ...
    specar.push({ fil: 'k6-' + b[1].toLowerCase() + (b[3] === 'AI-bild' ? '.webp' : '.svg'), underdel: BOK[+b[5] - 1], namn: b[2].trim(), placering: b[6].trim(), alt: f('Alt'), enkel: f('Bildtext Enkel'), standard: f('Bildtext Standard'), punkter, ankareStandard: 'konfig' });
  }
  ...
  if (finns(`avsnitt-${N}-enkel-fordjupning.md`)) {
    ...
    const ing = enkelDel.match(/## Avsnittsingress\n([\s\S]*?)\n---\n/);   // Enkel-ingressen levererad i nivåfilen (Alkoholer)
    if (ing && !INGRESS[String(N)]) { INGRESS[String(N)] = rensa(ing[1]); }
  ...
  let ut = `# Livets molekyler, avsnitt ${N} — ${titel}
## Byggfil, sammansatt av verktyg/satt-ihop-livets-molekyler.js ur avsnitt-${N}.md${enkel ? ` och avsnitt-${N}-enkel-fordjupning.md` : ' (bara Standard levererad – Enkel och Fördjupning utelämnade)'} – redigera inte här

**Sökväg:** \`kapitel/organisk-kemi/delkapitel/livets-molekyler/avsnitt-${N}-${HUVUD[N].slug}.html\`
...
if (finns('flipcards.md')) {
  ...
  let fut = `# Flipcards — delkapitel Livets molekyler (byggfil)\n\n> Sammansatt av verktyg/satt-ihop-livets-molekyler.js ur flipcards.md – redigera inte här.\n> Framsidan på begreppskorten är termen; baksidan definitionen. Bankformuleringar under Begreppsbanken.\n`;
...
if (finns('kortsvar.md')) {
  ...
  const TITLAR = {}; for (const N of [1, 2, 3, 4]) { if (finns(`avsnitt-${N}.md`)) TITLAR[N] = norm(`avsnitt-${N}.md`).match(/^# Avsnitt \d — ([^\n]+)/)[1].trim(); }
  let kut = `# Kortsvar — delkapitel Livets molekyler (byggfil)\n\n> Sammansatt av verktyg/satt-ihop-livets-molekyler.js ur kortsvar.md – redigera inte här. Blockform (KEMI-TILLAGG §8).\n`;
  ...
  for (const a of ks.matchAll(/\n# Avsnitt (\d)(?: — [^\n]*)?\n([\s\S]*?)(?=\n# Avsnitt |\n---\n\n## Att kontrollera|$)/g)) {
    ...
    for (const q of a[2].matchAll(/\*\*(\d+)\.\*\* `([a-z-]+)`\n([\s\S]*?)\n\*\*Svar:\*\* ([^\n]+)\n\*Förklaring:\* ([\s\S]*?)(?=\n\n\*\*\d+\.\*\*|\n*$(?![\s\S]))/g)) {
      ...
      // undantag ur ordern 2026-09-19 §6: fråga 42 godkänner mineralämne, mineralämnen och mineraler (rättningen tolererar bara
      // ändelser på elevens ord, så singularen och "mineraler" måste stå som alternativ); fråga 18 lipas/lipaser täcks av ändelseregeln
      if (typ === 'ord' && q[1] === '42') { S = 'mineralämnen | mineralämne | mineraler'; }
      const O = '';

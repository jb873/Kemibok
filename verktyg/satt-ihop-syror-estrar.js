// satt-ihop-syror-estrar.js – sätter ihop byggfiler för Organisk kemi 5 "Organiska syror och estrar" ur leveransfilerna
// (doc/leveranser/syror-och-estrar/, framtagna ur Joachims arbetsfiler av verktyg/rensa-syror-estrar.js):
...
//   original/dk5-bildspecar.md      (bildspecar: "## L1 — Titel" + "*1.1, vid första rubriken*", alla SVG)
//   knapptitlar-och-ingresser.md    (valfri, Joachims beslut), karnpunkter.md (valfri), flipcards.md/kortsvar.md (valfria)
// → doc/leveranser/syror-och-estrar/bygg/… i bygg-avsnitt.js-format. Bildankare per nivå ligger i bygg-avsnitt-konfig.js.
// Kör: node verktyg/satt-ihop-syror-estrar.js [N…], sedan node verktyg/bygg-avsnitt.js syror-och-estrar N.
// Kopia av satt-ihop-alkoholer.js (arbetsorder 2026-09-18) med k5-prefix och kortsvarsundantag (fråga 15 och 31); övriga regler lika.
'use strict';
const fs = require('fs'), path = require('path');
const { arReaktion } = require('./lib-notation.js');
const R = path.join(__dirname, '..', 'doc', 'leveranser', 'syror-och-estrar');
...
// slug och hero-underrubrik per avsnitt (ordern 2026-09-18: knapptitlar och hero-underrubriker levereras separat; förslag av
// Code, rapporterade – ersätts av knapptitlar-och-ingresser.md när Joachim beslutar)
const HUVUD = {
  1: { slug: 'karboxylgruppen', sub: 'gruppen som gör en molekyl sur' },
  2: { slug: 'organiska-syror-omkring-oss', sub: 'från myrans försvar till stearinljuset' },
  3: { slug: 'estrar', sub: 'när en alkohol och en syra möts' }
};
const BOK = ['A', 'B', 'C'];
...
if (finns('karnpunkter.md')) {
  ...
  const n = Object.values(KP).reduce((x, l) => x + l.split('\n').length, 0);   // ordern 2026-09-18: 9 underdelar × 5 = 45
  if (Object.keys(KP).length !== 9 || n !== 45) throw new Error(`karnpunkter.md: ${Object.keys(KP).length} underdelar, ${n} punkter (väntat 9 × 5 = 45)`);
...
for (const N of AVSNITT) {
  ...
  for (const u of under) {
    ...
    for (const d of delar.slice(1)) {
      ...
      const fil = 'k5-' + h[1].toLowerCase() + (h[3].startsWith('AI') ? '.webp' : '.svg');
  ...
  // bildspecar ur original/dk5-bildspecar.md: "## L1 — Titel" + "*1.1, vid första rubriken*" (alla SVG)
  const BSdel = fs.readFileSync(path.join(R, 'original', 'dk5-bildspecar.md'), 'utf8').replace(/\r\n/g, '\n');
  for (const b0 of BSdel.matchAll(/\n## ([A-Z]\d+) — ([^\n]+)\n\*(\d)\.(\d), vid ([^*]+?)\*\n([\s\S]*?)(?=\n---\n|\n# )/g)) {
    ...
    specar.push({ fil: 'k5-' + b[1].toLowerCase() + (b[3] === 'AI-bild' ? '.webp' : '.svg'), underdel: BOK[+b[5] - 1], namn: b[2].trim(), placering: b[6].trim(), alt: f('Alt'), enkel: f('Bildtext Enkel'), standard: f('Bildtext Standard'), punkter, ankareStandard: 'konfig' });
  }
  ...
  let ut = `# Organiska syror och estrar, avsnitt ${N} — ${titel}
## Byggfil, sammansatt av verktyg/satt-ihop-syror-estrar.js ur avsnitt-${N}.md${enkel ? ` och avsnitt-${N}-enkel-fordjupning.md` : ' (bara Standard levererad – Enkel och Fördjupning utelämnade)'} – redigera inte här

**Sökväg:** \`kapitel/organisk-kemi/delkapitel/syror-och-estrar/avsnitt-${N}-${HUVUD[N].slug}.html\`
...
if (finns('flipcards.md')) {
  ...
  const TITLAR = {}; for (const N of [1, 2, 3]) { if (finns(`avsnitt-${N}.md`)) TITLAR[N] = norm(`avsnitt-${N}.md`).match(/^# Avsnitt \d — ([^\n]+)/)[1].trim(); }
  let fut = `# Flipcards — delkapitel Organiska syror och estrar (byggfil)\n\n> Sammansatt av verktyg/satt-ihop-syror-estrar.js ur flipcards.md – redigera inte här.\n> Framsidan på begreppskorten är termen; baksidan definitionen. Bankformuleringar under Begreppsbanken.\n`;

// satt-ihop-alkoholer.js – sätter ihop byggfiler för Organisk kemi 4 "Alkoholer" ur leveransfilerna
// (doc/leveranser/alkoholer/, framtagna ur Joachims arbetsfiler av verktyg/rensa-alkoholer.js):
...
//   original/dk4-omskrivet-och-bildspecar.md  (bildspecar efter "# Bildspecar": "### K1 — Titel" + "*1.1, vid andra rubriken*", alla SVG)
//   knapptitlar-och-ingresser.md    (valfri, Joachims beslut), karnpunkter.md (valfri), flipcards.md/kortsvar.md (valfria)
// → doc/leveranser/alkoholer/bygg/… i bygg-avsnitt.js-format. Bildankare per nivå ligger i bygg-avsnitt-konfig.js.
// Kör: node verktyg/satt-ihop-alkoholer.js [N…], sedan node verktyg/bygg-avsnitt.js alkoholer N.
// Kopia av satt-ihop-fossila.js (arbetsorder 2026-09-18) med bildspecar ur omskrivet-filen, Enkel-ingress ur nivåfilen och
// k4-prefix; övriga regler lika.
'use strict';
const fs = require('fs'), path = require('path');
const { arReaktion } = require('./lib-notation.js');
const R = path.join(__dirname, '..', 'doc', 'leveranser', 'alkoholer');
...
for (const N of AVSNITT) {
  ...
  for (const u of under) {
    ...
    for (const d of delar.slice(1)) {
      ...
      const fil = 'k4-' + h[1].toLowerCase() + (h[3].startsWith('AI') ? '.webp' : '.svg');
  ...
  // bildspecar ur original/dk4-omskrivet-och-bildspecar.md efter "# Bildspecar": "### K1 — Titel" + "*1.1, vid andra rubriken*" (alla SVG)
  const BS = fs.readFileSync(path.join(R, 'original', 'dk4-omskrivet-och-bildspecar.md'), 'utf8').replace(/\r\n/g, '\n');
  const BSdel = BS.slice(BS.indexOf('\n# Bildspecar\n'));
  for (const b0 of BSdel.matchAll(/\n### ([A-Z]\d) — ([^\n]+)\n\*(\d)\.(\d), vid ([^*]+?)\*\n([\s\S]*?)(?=\n---\n|\n# )/g)) {
    ...
    specar.push({ fil: 'k4-' + b[1].toLowerCase() + (b[3] === 'AI-bild' ? '.webp' : '.svg'), underdel: BOK[+b[5] - 1], namn: b[2].trim(), placering: b[6].trim(), alt: f('Alt'), enkel: f('Bildtext Enkel'), standard: f('Bildtext Standard'), punkter, ankareStandard: 'konfig' });
  }
  ...
  let ut = `# Alkoholer, avsnitt ${N} — ${titel}
## Byggfil, sammansatt av verktyg/satt-ihop-alkoholer.js ur avsnitt-${N}.md${enkel ? ` och avsnitt-${N}-enkel-fordjupning.md` : ' (bara Standard levererad – Enkel och Fördjupning utelämnade)'} – redigera inte här

**Sökväg:** \`kapitel/organisk-kemi/delkapitel/alkoholer/avsnitt-${N}-${HUVUD[N].slug}.html\`
...
if (finns('flipcards.md')) {
  ...
  let fut = `# Flipcards — delkapitel Alkoholer (byggfil)\n\n> Sammansatt av verktyg/satt-ihop-alkoholer.js ur flipcards.md – redigera inte här.\n> Framsidan på begreppskorten är termen; baksidan definitionen. Bankformuleringar under Begreppsbanken.\n`;
...
if (finns('kortsvar.md')) {
  ...
  const TITLAR = {}; for (const N of [1, 2, 3]) { if (finns(`avsnitt-${N}.md`)) TITLAR[N] = norm(`avsnitt-${N}.md`).match(/^# Avsnitt \d — ([^\n]+)/)[1].trim(); }
  let kut = `# Kortsvar — delkapitel Alkoholer (byggfil)\n\n> Sammansatt av verktyg/satt-ihop-alkoholer.js ur kortsvar.md – redigera inte här. Blockform (KEMI-TILLAGG §8).\n`;
  ...
  for (const a of ks.matchAll(/\n# Avsnitt (\d)(?: — [^\n]*)?\n([\s\S]*?)(?=\n# Avsnitt |\n---\n\n## Att kontrollera|$)/g)) {
    ...
    for (const q of a[2].matchAll(/\*\*(\d+)\.\*\* `([a-z-]+)`\n([\s\S]*?)\n\*\*Svar:\*\* ([^\n]+)\n\*Förklaring:\* ([\s\S]*?)(?=\n\n\*\*\d+\.\*\*|\n*$(?![\s\S]))/g)) {
      ...
      // undantag ur leveransens "Att kontrollera" (Öva-ordern Alkoholer 2026-09-18 §2): fråga 15 svar 15 men 12 ska godkännas → lista
      if (typ === 'tal' && q[1] === '15') { S = '15 | 12'; }

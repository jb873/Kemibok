...
// kapitel ur konfigurationen (DELKAPITEL[dk].kapitel), standard Syror och baser; banken byggs bara ur delkapitel i samma kapitel
const KAPITEL = DELKAPITEL[DKID].kapitel || { id: 'syror-och-baser', titel: 'Syror och baser' };
const KAP = KAPITEL.id;
const levMapp = dk => path.join(ROT, 'doc', 'leveranser', dk, DELKAPITEL[dk].byggmapp || '');   // ev. undermapp med sammansatta byggfiler
...
function lasLeverans(dk) {
  ...
  const bankTerm = {};   // kortets titel → annan term i banken ("**C₆₀** → **Buckminsterfulleren** — …", kolatomen)
  if (slut > 0) { for (const m of bankSek.matchAll(/\n\*\*([^*]+)\*\*(?: → \*\*([^*]+)\*\*)? — ([\s\S]*?)(?=\n\n|\n*$(?![\s\S]))/g)) { const k = m[1].trim().toLowerCase(); utanFormel[k] = m[3].replace(/\n/g, ' ').trim(); if (m[2]) { bankTerm[k] = m[2].trim().toLowerCase(); } } }
  const avsnitt = {};
  ...
  return { avsnitt, utanFormel, bankTerm };
}
...
fs.mkdirSync(path.join(ROT, 'kapitel', KAP, 'data', 'flipcards'), { recursive: true });
// nytt kapitel: banken skapas med samma huvud som Syror och baser (kapitel_id/titel byts)
const bank = fs.existsSync(bankFil) ? JSON.parse(fs.readFileSync(bankFil, 'utf8'))
  : Object.assign(JSON.parse(fs.readFileSync(path.join(ROT, 'kapitel', 'syror-och-baser', 'data', 'begreppsbank.json'), 'utf8')), { kapitel_id: KAP, kapitel_titel: KAPITEL.titel, begrepp: [] });   // kapitel_id, titel, upplasning m.m. behålls
const nya = [], summering = [];
for (const dk of Object.keys(DELKAPITEL)) {
  if ((DELKAPITEL[dk].kapitel || { id: 'syror-och-baser' }).id !== KAP) { continue; }   // bara samma kapitel
  ...
  for (const [N, a] of Object.entries(L.avsnitt)) {
    ...
    for (const t of a.termer) {
      ...
      nya.push({ id: t.id.replace(/^k/, 'k' + B.idPrefix), avsnitt: String(+N + B.avsnittOffset), avsnitt_titel: K.titel, delkapitel_titel: DELKAPITEL[dk].titel,
        term: L.bankTerm[t.term] || t.term, expertdefinition: def, kallfil: `kapitel/${KAP}/data/flipcards/avsnitt-${N}-${K.slug}.json` });
      antal++;
    }
  }

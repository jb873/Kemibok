...
| 2026-09-13 | B | Kemibok, avsnitt 3 C (flerradiga bildtexter) | `figcaption` i `.brodtext-bild` är centrerad (ärver `text-align:center`), kursiv, 0.9 rem och `--muted` (5.5:1 på paper – klarar AA). Över två rader hoppar vänsterkanten och texten blir svårläst; de tre dämpningarna (mindre, kursiv, ljusare) samverkar. Samma i Geografi och Historia. | Vänsterställ flerradiga bildtexter men behåll centrering för enradiga: `.brodtext-bild figcaption { display:inline-block; text-align:left; }` (enradiga krymper till innehållet och centreras av figuren, flerradiga fyller bredden). Överväg `--ink` i stället för `--muted` på bildtext. Plattformsvärde – ändras i alla böcker samtidigt. |

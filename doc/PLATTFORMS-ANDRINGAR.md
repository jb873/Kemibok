...
| 2026-09-18 | B | Kemibok, Alkoholer (kortsvar, Öva-ordern) | `js/kortsvar-gradering.js` (delad rättningsmotor för alla kemins kortsvar) godtar nu en **lista med exakt godkända värden** för typen `tal` (`svar: [15, 12]`, i leveransen `S: 15 | 12`); facit visar det första. Behövdes när texten säger "ungefär tolv till femton procent" – en tolerans hade också godkänt 18. Samma dag byttes Organiska syror och estrars fråga 15 (fem till sex procent) från tolerans ±1 till listan `6 | 5`, som annars hade godkänt 7. `verktyg/bygg-kortsvar.js` läser listan, självtestet rättar mot första värdet. | Regeln återkommer varje gång en text anger ett intervall. Bör följa med om graderaren lyfts till plattform. |
